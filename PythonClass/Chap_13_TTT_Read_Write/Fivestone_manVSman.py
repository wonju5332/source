...
# 초기 바둑판 좌표값
def emptyboard():
    empty_board = [[EMPTY for i in range(9)] for i in range(9)]  # empty를 9x9에 때려박음
...
class human():
    ...
    def Action(self, state):
        ...
        for i in range(1, 10):
            for j in range(1, 10):
                switch_map[10 * i + j] = (i, j)  # Player input = 55  ---> (5,5) 로 변환

                # 인풋 받기
        while action not in range(11, 100) or state[switch_map[action][0] - 1][switch_map[action][1] - 1] != EMPTY:  #action 는 ex) 55, 82, 91...
                                              # 코드의 에러 중 하나인 같은곳에 두게되면 돌이 바뀌어버리는 경우가 있었음
                                              # 방지하기 위해, 위 코드를 작성함.
                                              # 기존에 돌을 두었는지, 안두었는지 확인하는 것.
            try:
                action = int(input('Player{}의 차례입니다. '.format(self.player)))
            except ValueError:
                continue

        return switch_map[action]      #55를 입력하면 (5,5)로 리턴
