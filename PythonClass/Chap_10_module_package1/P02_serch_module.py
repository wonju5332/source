"""
방금 만든 calculator 모듈의 위치는 우리가 직접 지정한 위치에 저장되어 있었고, calc_test.py에서 calculator 모듈을 불러올 수 있었다.

sys라는 모듈 ( ex) random함수 포함하고 있는 모듈) 은 어디에 있는 것인가?

Python은 import를 만나면 아래와 같은 순서로 모듈 파일을 찾아 나선다.

    1. Python 인터프리터 내장 모듈
    2. sys.path에 정의 되어있는 DIRECTORY
...
# 예제 ) sys의 builtin_module_names안에 랜덤함수가 있는지 확인해봐라.
...
#예제)sys.path의내용을출력해서파이썬이어떻게모듈을탐색해나가는지확인하라.
