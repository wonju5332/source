...
"""
                                                ▶ 3.16  시간대 관련 날짜 처리◀ 
♣  문제 : 시카고 시간으로 2012년 12월 21일 오전 9시 30분에 화상회의가 예정되어 있다. 
            그렇다면 인도와 방갈도르에 있는 친구는 몇시에 회의실에 와 있을까?
            
↘  해결 :  시간대 관련된 문제는 pytz모듈로 거의 해결한다. 이 패키지는 많은 언어운영체제에서 기본적으로 사용하는
            Olson 시간대 DB를 제공한다.
            pytz는 주로 datetime 라이브러리에서 생성한 날짜를 현지화할 때 사용한다. 
...
# 위처럼 날짜를 현지화하고 나면, 다른 시간대로 변환할 수 있다.
# 방갈로르와 동일 시간을 구하려면 다음과 같이 한다.

bang_d = loc_d.astimezone(timezone('Asia/Kolkata'))
print(bang_d)  #2012-12-21 21:00:00+05:30


#변환한 날짜에 산술연산을 하려면 서머타임제 등을 알고 있어야 한다.
#ex) 2013년 미국에서 표준서머타임은 3월 13일 오전 2시에 시작한다(여기서 한시간 생략한다)
#이를 고려하지 않고 계산하면 계산결과가 잘못된다.

d = datetime(2013,3,10,1,45)
print(d)
loc_d = central.localize(d)   #2013-03-10 01:45:00-06:00
print(loc_d)

later = loc_d + timedelta(minutes=30)   #2013-03-10 02:15:00-06:00     <틀린 결과값임
print(later)

# 위 틀린 결과를 수정하려면 노멀화(nomalization)을 해야한다.

from datetime import timedelta
later = central.normalize(loc_d + timedelta(minutes=30))  #2013-03-10 03:15:00-05:00
print(later)


# 현지화된 날짜를 조금 더 쉽게 다루기 위한 한가지 전략으로 모든 날짜를 UTC 시간으로 변환해놓고 사용하는 게 있다.

print(loc_d)
utc_d = loc_d.astimezone(pytz.utc)    #2013-03-10 07:45:00+00:00
print(utc_d)

# UTC에서는 서머타임과 같은 자잘한 문제를 신경쓰지 않아도 된다.
# 따라서 일반적인 산술을 날짜에 사용해도 문제가 없다.
# 현지화 시간을 출력해야 한다면 모든계산을 마친 후에 원하는 시간대로 변환하면 그만이다.

later_utc = utc_d+timedelta(minutes=30)
print(later_utc.astimezone(central))


# 시간대 관련 작업 중 챙기는 문제 중 한가지로 , 어떤 이름을 사용할 지 결정하는 것이 있다.
# 예를 들어, 이번 레시피에서 인도의 시간대 이름이 'Asia/Kolkata' 라는 것을 어떻게 알아냈을까!
# 하는 점은 pytz.country_timezones 딕셔너리를 사용한다.
# 이때 키 값으로는 ISO 3166 국가코드를 사용한다.

a = pytz.country_timezones['IN']  #['Asia/Kolkata']
print(a)
